...
#define TEXTURE_FILE "lena_greyscale.bmp"
...
void init_cl() {
   ...
   /* Read pixel data */
   input_image = readGreyscaleImage(TEXTURE_FILE, &w, &h);
   width = w;
   height = h;

   /* Create the input image object from the image data */
   img_format.image_channel_order = CL_R;
   img_format.image_channel_data_type = CL_UNSIGNED_INT8;
   in_texture = clCreateImage2D(context, 
         CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
         &img_format, width, height, 0, (void*)input_image, &err);
   if(err < 0) {
